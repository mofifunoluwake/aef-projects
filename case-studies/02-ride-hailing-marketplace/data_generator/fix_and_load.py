...
tables = build_tables(rng, 15000)

# Convert all datetime columns to ISO strings so write_pandas doesn't corrupt them
for name, df in tables.items():
    for col in df.columns:
        if df[col].dtype.kind == "M":  # datetime-like column
